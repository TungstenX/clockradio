...
class SPIClient:
    ...
    def ili_init(self):
        ...
        self.send_cmd(0xE0); self.send_data_bytes([0x00,0x07,0x0F,0x0D,0x1B,0x0A,0x3C,0x78,0x4A,0x07,0x0E,0x09,0x1B,0x1E,0x0F])
        self.send_cmd(0xE1); self.send_data_bytes([0x00,0x22,0x24,0x06,0x12,0x07,0x36,0x47,0x47,0x06,0x0A,0x07,0x30,0x37,0x0F])
        self.send_cmd(0xC0); self.send_data_bytes([0x17, 0x15])
        self.send_cmd(0xC1); self.send_data_bytes([0x41])
        self.send_cmd(0xC5); self.send_data_bytes([0x00, 0x12, 0x80])
        # Pixel Format: 0x66 -> 18-bit (RGB666)
        self.send_cmd(0x3A); self.send_data_bytes([0x66])
        # Memory Access Control: orientation (may need change)
        self.send_cmd(0x36); self.send_data_bytes([0x48])
        self.send_cmd(0x11); time.sleep(0.12)   # Sleep Out
        self.send_cmd(0x29); time.sleep(0.05)   # Display ON
