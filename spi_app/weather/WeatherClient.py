...
class WeatherClientCR:
    ...
    def fetch_weather(self):
        ...
        complete_url = ("https://api.weatherapi.com/v1/forecast.json?key=" + self.config.get("Weather", "api_key") +
                        "&q=-26.063018646041453,27.961667533360554&aqi=no&days=3")
        try:
            ...
            if response.status_code == 200:
                ...
                self.sun_and_moon(data, dt_now)
                # Current conditions
                self.calc_tod(data, dt_now)
            else:
                self.logger.error("City Not Found")
        except Exception as e:
            self.logger.error(f"Error while fetching_weather:\n{response}\n{e}")

    def calc_tod(self, data, dt_now: datetime):
    ...
    def sun_and_moon(self, data, dt_now: datetime):
        ...
        self.forecast["today"]["sun"]["is_up"] = self.forecast["today"]["sun"]["rise"] <= dt_now <= \
                                                 self.forecast["today"]["sun"]["set"]
        self.forecast["today"]["moon"]["is_up"] = self.forecast["today"]["moon"]["rise"] <= dt_now <= \
                                                  self.forecast["today"]["moon"]["set"]

        if self.forecast["today"]["sun"]["is_up"]:
            self.daylight_minutes_total = (self.forecast["today"]["sun"]["set"] - self.forecast["today"]["sun"][
                "rise"]).seconds / 60
            ...
            if dt_now < self.forecast["today"]["sun"]["rise"]:
                ...
                self.night_minutes_total = (self.forecast["today"]["sun"]["rise"] - self.forecast["today"]["sun"][
                    "set"]).seconds / 60
        ...
        if self.forecast["today"]["moon"]["is_up"]:
            self.moon_minutes_total = (self.forecast["today"]["moon"]["set"] - self.forecast["today"]["moon"][
                "rise"]).seconds / 60
